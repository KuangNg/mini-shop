...
import { getMenberOrderPreAPI, getMemberOrderPreNowAPI, postMemberOrderAPI } from '@/services/order'
...
// 预支付订单接口获取
const getMenberOrderPreData = async () => {
  // 是否有立即购买参数
  if (query.skuId && query.count) {
    // 调用立即购买 API
    const res = await getMemberOrderPreNowAPI({ skuId: query.skuId, count: query.count.toString() })
    orderPre.value = res.result
  } else {
    // 调用预付订单 API
    const res = await getMenberOrderPreAPI()
    orderPre.value = res.result
  }
}
// 地址仓库
...
const defaultAddress = computed(
  (): AddressItem =>
    orderPre.value?.userAddresses.find((item) => item.isDefault) || addressStore.defaultAddress,
)
// 提交订单
const onOrderSubmit = async () => {
  ...
  const res = await postMemberOrderAPI({
    ...
    deliveryTimeType: deliveryList.value[2].type,
...
<template>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="button" :class="{ disabled: orderPre?.goods.length === 0 }" @tap="onOrderSubmit">
      提交订单
    </view>
  </view>
</template>
