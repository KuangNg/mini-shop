<template>
  ...
  <scroll-view
    ...
    @refresherrefresh="scrolltoupperList"
...
const getMemberOrderData = async () => {
  ...
  // 请求接口
  const res = await getMemberOrderAPI(pageParams)
...
const onOrderPay = async (id: string) => {
  ...
  const order = orderList.value.map((v) => {
    if (v.id === id) {
      v.orderState = OrderState.DaiFaHuo
    }
    return v
  })
  orderList.value = JSON.parse(JSON.stringify(order))
