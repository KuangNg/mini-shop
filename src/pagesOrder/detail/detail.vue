...
// 获取订单物流信息 getMemberOrderLogisticsByIdAPI
const orderLogisticsList = ref<LogisticItem[]>([])
...
<template>
  ...
  <scroll-view scroll-y class="viewport" id="scroller" @scrolltolower="listScrolltolower">
    <template v-if="order">
      <!-- 订单状态 -->
      ...
      <view class="shipment">
        <!-- 订单物流信息 -->
        <view v-for="item in orderLogisticsList" :key="item.id" class="item">
          <view class="message">
            {{ item.text }}
          </view>
